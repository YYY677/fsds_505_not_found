...
kmeans = KMeans(n_clusters=3, random_state=42)  # Set K=3 for clustering
final_data['Cluster'] = kmeans.fit_predict(X_scaled)  # Add cluster labels to the DataFrame
...
# Load Airbnb data with latitude and longitude
airbnb_df = pd.read_csv('data/filtered_airbnb_data_with_geometry.csv')
airbnb_df['geometry'] = airbnb_df.apply(lambda row: Point(row['longitude'], row['latitude']), axis=1)
airbnb_gdf = gpd.GeoDataFrame(airbnb_df, geometry='geometry', crs="EPSG:4326")

# Ensure the CRS of all data matches
...
# Define softer cluster colors
cluster_colors = {0: '#FFB6C1', 1: '#ADD8E6', 2: '#90EE90'}  # Light pink, light blue, light green

# Plot LSOA regions colored by cluster
for cluster, color in cluster_colors.items():
    lsoa_gdf[lsoa_gdf['Cluster'] == cluster].plot(ax=ax, color=color, edgecolor='grey', linewidth=0.3, label=f'Cluster {cluster}')

# Plot Airbnb point data with transparency and smaller size
airbnb_gdf.plot(ax=ax, markersize=1, color='purple', alpha=0.3, label='Airbnb Listings')

# Adjust the extent (zoom to the full London area)
ax.set_xlim(lsoa_gdf.total_bounds[0] - 0.01, lsoa_gdf.total_bounds[2] + 0.01)
ax.set_ylim(lsoa_gdf.total_bounds[1] - 0.01, lsoa_gdf.total_bounds[3] + 0.01)

# Clean up the layout
ax.set_title("K-Means Clustering Results with Airbnb Listings", fontsize=14)
ax.axis("off")
